...
sPass = Request.ServerVariables("AUTH_PASS")

' The shared secret between this ASP page and ]po[
sSecret = "<enter_same_random_string_as_in_login.asp>"


' Calculate a YYYY-MM-DD datestamp
Dim sDate, sDay, sMonth
sDay = Cstr(day(date()))
...
' Calculate a HH:MM timestamp
Dim sTime, sHour, sMinute, sSecond
sHour = Cstr(hour(time()))
if len(sHour) < 2 then sHour = "0"+sHour
sMinute = Cstr(minute(time()))
if len(sMinute) < 2 then sMinute = "0"+sMinute
sSecond = Cstr(second(time()))
if len(sSecond) < 2 then sSecond = "0"+sSecond
sTime = sHour+"-"+sMinute+"-"+sSecond


' Calculate the part of the URL to be hashed:
Dim sCritical, sHash
sCritical = sSecret+sUser+sDate+sTime


' Calculata a SHA-1 Hash of the critical part of the URL
sHash = ucase(hex_sha1(sCritical))


' Calculate the URL
Dim sUrl
sUser = Replace(sUser, "\", "%5C")
sUrl = "https://calpms.mnet.moravia-it.com/moravia-login?username="+sUser+"&date="+sDate+"&time="+sTime+"&sha1="+sHash
