...
-------------------------------------------------------------
-- Load the Content-Repository create-templates.sql file
-- to updated PG 9.2 procedures
--
...
-------------------------------------------------------------
-- Load the postgresql.sql file with PG 9.x updates
--
...
create or replace view acs_func_defs as 
select get_func_definition(proname::varchar,proargtypes) as definition, 
       proname as fname 
  from pg_proc;

create or replace view acs_func_headers as 
