...
create or replace function im_project_id_parent_list(int4,varchar,int4) 
returns varchar as $body$
    DECLARE
	p_project_id		alias for $1;
...
DECLARE
	...
	p_exclude_main_project_p	alias for $2;
	p_exclude_sub_project_p		alias for $3;
...
BEGIN
	...
	WHILE v_ctr < 20 LOOP
		...
		IF v_parent_id is null THEN
			...
			IF not p_exclude_main_project_p THEN
				IF v_path != '' THEN v_path := ' / ' || v_path; END IF;
				v_path := v_project_name || v_path;
			END IF;
			EXIT; 
		END IF;
